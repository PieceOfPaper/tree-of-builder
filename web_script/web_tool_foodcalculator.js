    ...
    route.get('/', function (request, response) {
        ...
        for (var i = 0; i < foodTable.length; i ++){
            ...
            for (var j=0;j<material.length;j+=2){
                resultString += tos.GetItemResultString(serverData, material[j], serverData['imagePath'], '<span class="matcnt" id="matcnt-'+i+'-'+material[j]+'">'+material[j+1]+'</span>');
        ...
        for (var j=0;j<totalMaterials.length;j++){
            resultString += tos.GetItemResultString(serverData, totalMaterials[j], serverData['imagePath'], '<span class="matcnt" id="totalmatcnt-'+totalMaterials[j]+'">'+0+'</span>');
        }
